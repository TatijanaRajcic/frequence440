var prestations = [
  {
    category: "diagnostics",
    options: [
      {
        title: "Bilan carbone ®",
        content: "Selon la méthode Association Bilan Carbone",
      },
      {
        title: "Diagnostic RSE",
        content:
          "Avec la grille des Objectifs de Développement Durable à partir de l’outil  développé par l’Université Technolo- gique de Compiègne.",
      },
      {
        title: "Diagnostic équipes",
        content:
          "Niveau de connaissance et d’engagement de vos équipes. Recherche d’ambassadeur",
      },
    ],
  },
  {
    category: "formations",
    options: [
      {
        title: "Fresque du Climat",
        content:
          "Atelier créatif par Cédric Ringenbach, basé sur les rapports du GIEC pour comprendre en intelligence collective les mécanismes du climat et les interactions avec les activités humaines",
      },
      {
        title: "Atelier 2 Tonnes",
        content:
          "Une simulation interactive basée sur les bilans carbone des participants, (objectif : 2 tonnes pour la neutralité carbone en 2050). Pour comprendre les ordres de grandeur des actions pour le climat et l’interaction individu / collectif",
      },
      {
        title: "Renaissance Ecologique",
        content:
          "Atelier créatif et inspirant autour d’un récit et d’une illustration d’un futur souhaitable, inspiré de la fresque de Sienne de 1338 et créé par Julien Dossier, pour comprendre les interactions et le chemin à mener pour un monde bas carbone",
      },
      {
        title: "Fresque Biodiversité",
        content:
          "Atelier créatif basé sur le rapport de l’IPBES pour comprendre en intelligence collective les mécanismes de la biodiversité et comment limiter son effondrement",
      },
      {
        title: "Fresque Océane",
        content:
          "Atelier créatif pour tout comprendre sur les océans, les menaces qu’ils subissent et les solutions correspondantes",
      },
      {
        title: "Fresque du Numérique",
        content:
          "Atelier interactif pour comprendre les impacts du numérique et réfléchir sur les solutions pour un numérique durable",
      },
    ],
    addOn: {
      title: "Des formations en intelligence collective, par des experts",
      content:
        "Possibilité de former des ambassadeurs parmi vos collaborateurs ",
    },
  },
  {
    category: "conférences",
    options: [
      { title: "Les trois courbes", content: "Panorama de la transition" },
      {
        title: "Les trois courbes - Ingénieurs",
        content:
          "Panorama de la transition, avec les orientations que peuvent prendre des ingénieurs",
      },
      {
        title: "Elle est jolie ta robe aujourd’hui !",
        content: "Les sujets épineux du sexisme au travail",
      },
      { title: "Conférence personnalisée", content: "Suivant vos attentes..." },
    ],
    addOn: {
      title: "Des conférences interactives avec questionnaires instantanés",
      content: "Niveau technique au choix",
    },
  },
  {
    category: "ateliers",
    options: [
      {
        title: "Ateliers solutions",
        content:
          "Accompagnement de vos équipes dans la co-construction d'objectifs et de plans d'action. Posture de facilitatrice pour encourager l'intelligence collective et l'innovation.",
      },
      {
        title: "Collective impact",
        content:
          "Accompagnement pour lancer une initiative dans votre communauté locale en partenariat avec les habitants, associations, artistes, collectivités territoriales et autres entreprises locales",
      },
    ],
  },
];

export default prestations;
